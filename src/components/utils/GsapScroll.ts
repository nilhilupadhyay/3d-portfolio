...
export function setCharTimeline(
  ...
  let screenLight: THREE.Object3D | undefined;
  let monitor: THREE.Object3D | undefined;
  character?.children.forEach((object) => {
    if (object.name === "Plane004") {
      object.children.forEach((child) => {
        const material = (child as { material?: EmissiveMaterial }).material;
        if (!material) return;
  ...
  if (window.innerWidth > 1024) {
    if (character) {
      ...
      tl2
        ...
        .to(
          (monitor as { material?: EmissiveMaterial } | undefined)?.material ??
            {},
          { opacity: 1, duration: 0.8, delay: 3.2 },
          0
        )
        .to(
          (screenLight as { material?: EmissiveMaterial } | undefined)
            ?.material ?? {},
